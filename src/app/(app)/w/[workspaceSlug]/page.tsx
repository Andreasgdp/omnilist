...
export default async function WorkspaceOverviewPage({
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] border border-border/60 bg-[linear-gradient(135deg,color-mix(in_oklab,var(--primary)_16%,white),color-mix(in_oklab,var(--accent)_18%,white)_45%,color-mix(in_oklab,var(--secondary)_40%,white))] p-8 shadow-sm dark:bg-[linear-gradient(135deg,color-mix(in_oklab,var(--primary)_20%,transparent),color-mix(in_oklab,var(--accent)_20%,transparent)_45%,color-mix(in_oklab,var(--secondary)_28%,transparent))]">
        <div className="grid gap-8 lg:grid-cols-[1.05fr_1.2fr] lg:items-center">
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              <Link className="inline-flex h-11 items-center rounded-full bg-foreground px-5 text-sm font-medium text-background shadow-sm transition hover:opacity-90 dark:bg-primary dark:text-primary-foreground" href={routes.newList(workspace.slug)}>
                Create custom list
              </Link>
              <Link className="inline-flex h-11 items-center rounded-full border border-border/70 bg-background/70 px-5 text-sm font-medium text-foreground transition hover:bg-background dark:bg-card/70" href={routes.workspaceLists(workspace.slug)}>
                Browse lists
              </Link>
            </div>
          </div>

          <div className="rounded-[1.75rem] border border-white/40 bg-white/60 p-5 shadow-xl shadow-primary/10 backdrop-blur dark:border-white/10 dark:bg-card/80">
              ...
              {lists.slice(0, 4).map((list, index) => {
                ...
                return (
                  <Link
                    ...
                    className={`block rounded-3xl border border-white/40 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${palette}`}
                  >
